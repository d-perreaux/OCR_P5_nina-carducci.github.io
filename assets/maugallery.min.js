...
element.wrap(`<div class='item-column mb-4${columnClasses}'></div>`)}else{console.error(`Columns should be defined as numbers or objects. ${typeof columns} is not supported.`)}},moveItemInRowWrapper(element){element.appendTo(".gallery-items-row")},responsiveImageItem(element){if(element.prop("tagName")==="IMG"){element.addClass("img-fluid")}},openLightBox(element,lightboxId){$(`#${lightboxId}`).find(".lightboxImage").attr("src",element.find("img").attr("src"));$(`#${lightboxId}`).modal("toggle")},prevImage(){let activeImage=null;$("picture.gallery-item").each(function(){if($(this).find("img").attr("src")===$(".lightboxImage").attr("src")){activeImage=$(this).find("img")}});let activeTag=$(".tags-bar span.active-tag").data("images-toggle");let imagesCollection=[];if(activeTag==="all"){$(".item-column").each(function(){if($(this).find("img").length){imagesCollection.push($(this).find("img"))}})}else{$(".item-column").each(function(){if($(this).find("picture").data("gallery-tag")===activeTag){imagesCollection.push($(this).find("img"))}})}
let index=0,next=null;$(imagesCollection).each(function(i){if($(activeImage).attr("src")===$(this).attr("src")){index=i-1}});next=imagesCollection[index]||imagesCollection[imagesCollection.length-1];$(".lightboxImage").attr("src",$(next).attr("src"))},nextImage(){let activeImage=null;$("picture.gallery-item").each(function(){if($(this).find("img").attr("src")===$(".lightboxImage").attr("src")){activeImage=$(this).find("img")}});let activeTag=$(".tags-bar span.active-tag").data("images-toggle");let imagesCollection=[];if(activeTag==="all"){$(".item-column").each(function(){if($(this).find("img").length){imagesCollection.push($(this).find("img"))}})}else{$(".item-column").each(function(){if($(this).find("picture").data("gallery-tag")===activeTag){imagesCollection.push($(this).find("img"))}})}
